...
class MistralProvider(BaseLLMProvider):
    """
    Провайдер для работы с Mistral API (mistral-small-latest, mistral-large-latest, codestral-latest).
    """
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None, base_url: Optional[str] = None):
        key = api_key or os.getenv("MISTRAL_API_KEY", "")
        selected_model = model or os.getenv("MISTRAL_MODEL", "mistral-small-latest")
        url = base_url or os.getenv("MISTRAL_BASE_URL", "https://api.mistral.ai/v1")
